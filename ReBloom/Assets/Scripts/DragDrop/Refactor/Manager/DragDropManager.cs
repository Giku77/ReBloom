...
/// <summary>
/// 드래그앤드롭 중앙 관리자 (Marker 기반)
/// 모든 드래그 상태와 로직을 한 곳에서 관리
/// </summary>
public class DragDropManager : MonoBehaviour
{
	public static DragDropManager I { get; private set; }
	...
	private void Awake()
	{
		...
		// ItemDropCalculator 자동 찾기
		if (itemDropCalculator == null)
		{
			itemDropCalculator = FindFirstObjectByType<ItemDropCalculator>();
			if (itemDropCalculator == null)
			{
		...
		//Debug.Log("[DragDropManager] 초기화 완료");
	}
	#endregion

	#region Public API
	public WorldStorage GetCurrentStorage() => currentStorage;
	...
	public void BeginDrag(DragContext context)
	{
		...
		{
			Debug.LogWarning("[DragDropManager] 유효하지 않은 컨텍스트입니다.");
			return;
		}
	...
	public void EndDrag(PointerEventData eventData)
	{
		if (currentDrag == null)
		{
			Debug.LogWarning("[DragDropManager] 드래그 컨텍스트가 없습니다.");
			return;
		}

		// RaycastAll로 모든 UI 요소 검색
		...
		if (bestZone != null && CanDrop(bestZone, currentDrag))
		{
		   ...
		   // Debug.Log($"드롭 성공: {bestZone.ZoneType} (슬롯: {bestZone.SlotIndex})");
		}
		else
		{
	...
	public void NotifyHover(PointerEventData eventData)
	{
		...
		// 드롭 가능 여부 판단
		bool canDrop = hoveredZone != null && CanDrop(hoveredZone, currentDrag);
	...
	private DropZoneMarker FindBestDropZone(List<RaycastResult> results)
	{
        ...
        {
            Debug.LogWarning("[DragDropManager] 유효한 DropZone을 찾지 못했습니다!");
        }
	...
	private bool CanDrop(DropZoneMarker zone, DragContext context)
	{
		switch (zone.ZoneType)
		{
			...
			case DropZoneType.Inventory:
				return context.IsFromInventory || context.IsFromStorage;
    ...
    private void HandleInventoryDrop(int targetSlot, DragContext context)
    {
        ...
        {
            // 같은 슬롯에 드롭: 무시
            if (context.SourceSlotIndex == targetSlot)
            {
        ...
        else if (context.IsFromStorage)
        {
            ...
            {
                Debug.LogWarning($"[DragDropManager] 스토리지에서 인벤토리로 이동 실패: {context.Item.itemName} (인벤토리 가득참?)");
            }
        }
        else
        {
    ...
    /// 장비슬롯 드롭 처리
    /// </summary>
    private void HandleEquipmentDrop(DragContext context)
	{
		if (inventoryData == null)
		{
			Debug.LogError("[DragDropManager] InventoryItemData를 찾을 수 없습니다!");
		...
		// 같은 슬롯에 드롭: 무시
		if (context.IsFromEquipment)
		{
			Debug.Log($"[DragDropManager] 인벤토리 같은 슬롯 드롭 무시");
		   // return;
		}
		...
		if (context.IsFromInventory)
		{
		   equipManager.EquipItem(context.Item.itemID);
			Debug.Log("인벤토리에서 장비슬롯으로 배치 시도");
		}
	}

	#endregion

	#region Helper Methods - 퀵슬롯
	/// <summary>
	/// 퀵슬롯 간 스왑
	/// </summary>
	private void SwapQuickSlots(int fromIndex, int toIndex)
	{
		...
		{
			Debug.LogError("[DragDropManager] QuickSlot 매니저가 없습니다!");
			return;
		}
	   ...
	   // Debug.Log($"퀵슬롯 스왑: [{fromIndex}] {fromItem.itemName} <-> [{toIndex}] {(toItem != null ? toItem.itemName : "빈 슬롯")}");
	...
	/// 인벤토리 -> 퀵슬롯 배치
	/// </summary>
	private void AssignToQuickSlot(int slotIndex, ItemBase item)
	{
		if (quickSlotManager == null)
		{
			Debug.LogError("[DragDropManager] QuickSlot 매니저가 없습니다!");
		...
		if (quantity <= 0)
		{
			Debug.LogWarning($"[DragDropManager] 인벤토리에 {item.itemName}이(가) 없습니다!");
			return;
		}
		...
		{
			Debug.Log($"{item.itemName}이(가) 슬롯 {existingSlotIndex}에 있어서 제거 후 슬롯 {slotIndex}에 재배치");
		...
		{
		  //  Debug.Log($"슬롯 {slotIndex}: {currentSlotItem.itemName} → {item.itemName} (교체)");
		...
		if (success)
		{
		   // Debug.Log($"{item.itemName} x{quantity}를 슬롯 {slotIndex}에 배치 완료");
		}
	}
	#endregion

	#region Helper Methods - 월드 드롭
	/// <summary>
	/// 드롭 위치 가져오기 (ItemDropCalculator 사용)
	/// </summary>
	private Vector3 GetDropPosition()
	{
		if (itemDropCalculator != null)
		{
			return itemDropCalculator.CalculateDropPosition();
		}
		...
		if (player != null)
		{
			return player.transform.position + player.transform.forward * 2f + Vector3.up * 1.5f;
		}

		Debug.LogError("[DragDropManager] 드롭 위치를 계산할 수 없습니다!");
		return Vector3.zero;
	}
	...
	/// 월드에 아이템 드롭 (async)
	/// </summary>
	private async UniTask DropToWorldAsync(ItemBase item, Vector3 dropPosition, int quantity)
	{
		if (itemSpawner == null)
		{
			Debug.LogError("[DragDropManager] ItemSpawner를 찾을 수 없습니다!");
			return;
		}
	   ...
	   // Debug.Log($"{item.itemName} x{quantity}개를 월드에 드롭했습니다.");
	}
	#endregion
}